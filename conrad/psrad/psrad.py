...
The last variables are indexed from the ground up, i.e. pp_fl_vr[:,0] is the pressure at the surface and pp_fl_vr[:,klev-1] is the TOA pressure."""
	...
	solib.psrad_standalone_setup_multi(kbdim, klev, C_icldlyr, C_cldlyr_lwc, C_cldlyr_iwc, C_cldlyr_frc, C_zenith, C_albedo, C_pp_sfc, C_tk_sfc, C_hgt_fl_vr, C_pp_fl_vr, C_tk_fl_vr, C_xm_h2o_vr, C_xm_o3_vr, C_xm_n2o_vr, C_xm_co_vr, C_xm_ch4_vr)
...
* par_dff_frc(float64[kbdim]): Diffuse photosynthetically active fraction"""
	...
	solib.psrad_get_sw_fluxes(C_htngrt, C_htngrt_clr, C_flxd_sw, C_flxd_sw_clr, C_flxu_sw, C_flxu_sw_clr, C_vis_frc_sfc, C_par_dn_sfc, C_nir_dff_frc, C_vis_dff_frc, C_par_dff_frc)
...
solib.psrad_get_strategy_lw.restype = c_int
def get_strategy_lw():
	"Returns (int32) the LW sampling strategy. Currently hard-coded."
	return solib.psrad_get_strategy_lw()

solib.psrad_get_strategy_sw.restype = c_int
def get_strategy_sw():
	"Returns (int32) the SW sampling strategy. Currently hard-coded."
	return solib.psrad_get_strategy_sw()

solib.psrad_get_num_gpts_lw.restype = c_int
def get_num_gpts_lw():
	"Returns (int32) the number of g-points in the LW sampling strategy. Currently hard-coded."
	return solib.psrad_get_num_gpts_lw()

solib.psrad_get_num_gpts_sw.restype = c_int
def get_num_gpts_sw():
	"Returns (int32) the number of g-points in the SW sampling strategy. Currently hard-coded."
	return solib.psrad_get_num_gpts_sw()
