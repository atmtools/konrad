...
class RRTMG(Radiation):
    ...
    def init_radiative_state(self, atmosphere, surface):
        ...
        climt.set_constants_from_dict(
            {"stellar_irradiance": {"value": self.solar_constant, "units": "W m^-2"}}
        )
        if self._is_mcica:
        ...
        for state0 in state_lw, state_sw:
            ...
            state0["air_pressure"] = DataArray(
                plev, dims=("mid_levels",), attrs={"units": "Pa"}
            )
        ...
        for surface_albedo in [
            ...
            state_sw[surface_albedo] = DataArray(
                np.array(float(surface.albedo)), attrs={"units": "dimensionless"}
            )
    ...
    def update_cloudy_radiative_state(self, cloud, state0, sw=True):
        ...
        lw_props = ("longwave_optical_thickness_due_to_cloud",)
    ...
    def update_aerosol_radiative_properties(self, aerosol, state_sw, state_lw):
        state_sw[
            "shortwave_optical_thickness_due_to_aerosol"
        ] = aerosol.optical_thickness_due_to_aerosol_sw
        state_sw[
            "single_scattering_albedo_due_to_aerosol"
        ] = aerosol.single_scattering_albedo_aerosol_sw
        state_sw["aerosol_asymmetry_parameter"] = aerosol.asymmetry_factor_aerosol_sw
        state_lw[
            "longwave_optical_thickness_due_to_aerosol"
        ] = aerosol.optical_thickness_due_to_aerosol_lw
    ...
    def update_radiative_state(self, atmosphere, surface, state0, sw=True):
        ...
        for climt_key, konrad_key in gas_name_mapping:
            ...
            state0[climt_key] = DataArray(
                vmr, dims=("mid_levels",), attrs={"units": "mole/mole"}
            )

        # Surface quantities
        state0["surface_temperature"] = DataArray(
            np.array(surface["temperature"][-1]), attrs={"units": "degK"}
        )
    ...
    def radiative_fluxes(self, atmosphere, surface, cloud, aerosol):
        ...
        self.update_radiative_state(atmosphere, surface, self._state_lw, sw=False)
        self.update_radiative_state(atmosphere, surface, self._state_sw, sw=True)
    ...
    def calc_radiation(self, atmosphere, surface, cloud, aerosol):
        ...
        if not self._is_mcica and not isinstance(cloud, ClearSky):
            if isinstance(cloud, CloudEnsemble):
                ...
                # aerosols have to be added to CloudEnsemble
                lw_fluxes = {}
                sw_fluxes = {}
        ...
        self["lw_htngrt_clr"] = np.expand_dims(
            lw_fluxes["air_temperature_tendency_from_longwave_assuming_clear_sky"].data,
            0,
        )
